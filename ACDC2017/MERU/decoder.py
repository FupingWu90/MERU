...
class PixelShuffle(nn.Module):
    ...
    def __init__(self, n_channels, scale):
        ...
        self.conv = nn.Conv2d(n_channels, n_channels * (scale ** 1), kernel_size=1)
...
def upsample(in_channels, out_channels, upscale, kernel_size=3):
    ...
    for i in range(int(math.log(upscale, 2))):
        layers.append(PixelShuffle(in_channels//(2**i), scale=2))

    layers.append(nn.Conv2d(in_channels//(2**3), out_channels, kernel_size=3,padding=1))
